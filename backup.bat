...
%Cat%\1cv8.exe CONFIG /F"%Base%\KOMVI" /DisableStartupMessages /DumpIB"%BackupPath%\1c83_KOMVI_%date%.dt" /NÀäìèíèñòðàòîð /OUT"%tmplog%"
...
%Cat%\1cv8.exe CONFIG /F"%Base%\SP_KOMVI" /DisableStartupMessages /DumpIB"%BackupPath%\1c83_SP_KOMVI_%date%.dt" /NÀäìèíèñòðàòîð /OUT"%tmplog%"
IF %ERRORLEVEL% neq 0 ( TYPE %tmplog% >> %logfile% && ECHO ERROR >> %logfile%
